...
const AutoLock = {
  ...
  trigger() {
    ...
    // Clear all form fields and always land on login form
    ['login-username', 'login-password',
     'reg-name', 'reg-username', 'reg-password', 'reg-confirm', 'reg-income'
...
function openDetail(id) {
  ...
  body.innerHTML = `
    ...
    <button class="btn-sm" style="margin-top:12px;width:100%" data-action="edit" data-txn-id="${txn.id}">✏️ Edit</button>
    <button class="detail-delete" data-action="delete" data-txn-id="${txn.id}">🗑 Delete Transaction</button>
  `;
...
function renderDashboard() {
  // Read selected month from dropdown (defaults to current month)
  ...
  if (filterVal) {
    ...
    if (!isNaN(fy) && !isNaN(fm)) { y = fy; m = fm - 1; }
  }
  ...
  const periodLabel = new Date(y, m).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' });
  const el = document.getElementById('current-month-badge');
  if (el) el.textContent = periodLabel;
  const bp = document.getElementById('balance-period');
  if (bp) bp.textContent = periodLabel;
  const cl = document.getElementById('cat-month-label');
  if (cl) cl.textContent = new Date(y, m).toLocaleDateString('en-IN', { month: 'long', year: 'numeric' });
  ...
  // Categories — sanitize before innerHTML
  const catTotals = {};
  ...
  if (sortedCats.length === 0) {
    ...
    catList.innerHTML = sortedCats.map(([cat, amount]) => {
      const safeCat  = Security.sanitizeLoaded(cat);
      const safeIcon = Security.sanitizeIcon(getCategoryIcon(cat));
      const rawColor = getCategoryColor(cat);
      const safeColor = Security.validateHexColor(rawColor) ? rawColor : '#6366f1';
      return `
      <div class="cat-item">
        <div class="cat-item-top">
          <span class="cat-name">${safeIcon} ${safeCat}</span>
          <span class="cat-amount">${fmt(amount)}</span>
        </div>
        <div class="cat-bar">
          <div class="cat-fill" style="width:${(amount/maxCat*100).toFixed(1)}%;background:${safeColor}"></div>
        </div>
      </div>`;
    }).join('');
  }
    ...
    return `
      <div class="catmgr-item" id="catmgr-item-${idx}">
        ...
        <div class="catmgr-actions">
          ${!c.builtIn
            ? `<button class="catmgr-del-btn" data-cat-idx="${idx}" title="Delete category">
...
function populateFilterDropdowns() {
  ...
  // Analytics + Transactions filters (include "All Time" option)
  ...
  // Dashboard month selector — no "All Time", defaults to current month
  const dashSel = document.getElementById('dashboard-month');
  if (dashSel) {
    const cur = dashSel.value || currentKey;
    dashSel.innerHTML = monthOptions.map(({ key, label }) =>
      `<option value="${key}" ${key === cur ? 'selected' : ''}>${label}</option>`
    ).join('');
    // Ensure a value is always selected
    if (!dashSel.value) dashSel.value = currentKey;
  }
}
...
function txnCard(t) {
  ...
  const icon     = Security.sanitizeIcon(getCategoryIcon(t.category));
  const rawColor = getCategoryColor(t.category);
  const color    = Security.validateHexColor(rawColor) ? rawColor : '#6366f1';
...
function renderAnalytics() {
  ...
  // Pass monthFilter so trend chart centres on the selected month
  renderTrendChart(monthFilter);
...
function renderTrendChart(monthFilter) {
  ...
  // Centre the 6-month window on the selected month (or current month for 'all')
  let anchor = new Date();
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    months.push({
      ...
      isSelected: monthFilter !== 'all' && key === monthFilter
    });
  }

  // Always use all transactions for the trend (not just filtered month)
  // so you can see context around the selected month
  ...
  // Dots + labels — selected month highlighted in amber
  points.forEach((p, i) => {
    const isSelected = months[i].isSelected;
    ctx.beginPath();
    ctx.arc(p.x, p.y, isSelected ? 6 : 4, 0, Math.PI * 2);
    ctx.fillStyle = isSelected ? '#f59e0b' : accentColor;
    ctx.fill();
    ctx.strokeStyle = isDark ? '#1e293b' : '#fff';
    ctx.lineWidth = 2;
    ctx.stroke();

    ctx.fillStyle  = isSelected ? '#f59e0b' : textColor;
    ctx.font       = isSelected ? 'bold 10px DM Sans, sans-serif' : '10px DM Sans, sans-serif';
...
function renderDonutChart(txns) {
  ...
  legend.innerHTML = entries.map(([cat, val]) => {
    const safeCat   = Security.sanitizeLoaded(cat);
    const safeColor = Security.validateHexColor(CATEGORY_COLORS[cat] || '') ? CATEGORY_COLORS[cat] : '#6366f1';
    return `
    <div class="legend-item">
      <div class="legend-dot" style="background:${safeColor}"></div>
      <span>${safeCat.split(' ')[0]} — ${Math.round(val/total*100)}%</span>
    </div>`;
  }).join('');
}
...
function renderBudget() {
  ...
  document.getElementById('rule-breakdown').innerHTML = [
    { label: 'Needs (50%)',   spent: needsSpent,   budget: needsBudget,   cls: 'needs',   color: 'var(--amber)', isSavings: false },
    { label: 'Wants (30%)',   spent: wantsSpent,   budget: wantsBudget,   cls: 'wants',   color: 'var(--blue)',  isSavings: false },
    { label: 'Savings (20%)', spent: savingsSpent, budget: savingsBudget, cls: 'savings', color: 'var(--green)', isSavings: true  }
  ].map(({ label, spent, budget, cls, color, isSavings }) => {
    const pct      = budget > 0 ? Math.min((spent / budget) * 100, 100) : 0;
    const over     = spent > budget;
    // Savings: going over is an achievement — green + trophy badge
    // Needs/Wants: going over is bad — red warning
    const barColor = over ? (isSavings ? 'var(--green)' : 'var(--red)') : color;
    const numColor = over ? (isSavings ? 'var(--green)' : 'var(--red)') : 'inherit';
    const badge    = over
      ? (isSavings
          ? `<span style="font-size:11px;margin-left:6px;background:var(--green-light);color:var(--green);padding:2px 7px;border-radius:10px;font-weight:600">🏆 +${fmt(spent - budget)} extra!</span>`
          : `<span style="font-size:11px;margin-left:6px;color:var(--red)">⚠ over by ${fmt(spent - budget)}</span>`)
      : '';
    return `<div class="rule-item">
      <div class="rule-item-top">
        <span class="rule-name">${label}${badge}</span>
        <span class="rule-nums" style="color:${numColor}">${fmt(spent)} / ${fmt(budget)}</span>
      </div>
      <div class="rule-bar"><div class="rule-fill ${cls}" style="width:${pct}%;background:${barColor}"></div></div>
    </div>`;
  }).join('');
  ...
  if (!Object.keys(STATE.budgets).length) {
    ...
    catBudgetsList.innerHTML = Object.entries(STATE.budgets).map(([cat, budget]) => {
      ...
      if (isNaN(safeBudget) || safeBudget <= 0) return '';
      const safeIcon   = Security.sanitizeIcon(getCategoryIcon(cat));
      const spent      = catSpending[cat] || 0;
      const pct        = Math.min((spent / safeBudget) * 100, 100);
      const over        = spent > safeBudget;
      return `<div class="cat-budget-item">
        <div class="cat-budget-top">
          <span class="cat-budget-name">${safeIcon} ${safeCat}</span>
          <span class="cat-budget-nums" style="color:${over?'var(--red)':'inherit'}">${fmt(spent)} / ${fmt(safeBudget)}</span>
        </div>
        <div class="cat-budget-bar"><div class="cat-budget-fill ${over?'over':''}" style="width:${pct}%"></div></div>
        ${over ? `<div class="cat-budget-warning">⚠ Over budget by ${fmt(spent - safeBudget)}</div>` : ''}
      </div>`;
    }).join('');
  }
...
function exportData() {
  const data = {
    ...
    budgets:      STATE.budgets,
    goals:        STATE.goals,
    categories:   (STATE.settings.categories || []).filter(c => !c.builtIn),
...
async function handleImportFile(event) {
  ...
  if (!file.name.endsWith('.json') && file.type !== 'application/json') {
    showToast('⚠ Please select a valid Expenseive JSON file.'); return;
  }
  if (file.size > 10 * 1024 * 1024) { showToast('⚠ File too large.'); return; }

  let parsed;
  try { parsed = JSON.parse(await file.text()); }
  catch { showToast('⚠ Could not read file — must be valid JSON.'); return; }

  if (!Array.isArray(parsed.transactions) || typeof parsed.settings !== 'object') {
    showToast('⚠ This doesn\'t look like an Expenseive export file.'); return;
  }

  const MAX_TXNS = 10000, MAX_GOALS = 500, MAX_CATS = 100;
  if (parsed.transactions.length > MAX_TXNS) {
    showToast(`⚠ Import exceeds ${MAX_TXNS} transaction limit.`); return;
  }

  const VALID_TYPES     = new Set(['Need','Want','Saving','Income']);
  const VALID_PAYMENTS  = new Set(['UPI','Credit Card','Debit Card','Cash','Bank Transfer','Net Banking','']);
  ...
  const safeDate = v => /^\d{4}-\d{2}-\d{2}$/.test(v) ? v : todayStr();

  const txnCount  = parsed.transactions.length;
  const goalCount = Array.isArray(parsed.goals)      ? parsed.goals.length                               : 0;
  const catCount  = Array.isArray(parsed.categories) ? parsed.categories.filter(c => !c.builtIn).length : 0;
  let exportedOn  = 'unknown date';
  if (parsed.exportedAt) {
    const d = new Date(parsed.exportedAt);
    if (!isNaN(d.getTime()))
      exportedOn = d.toLocaleDateString('en-IN', { day:'2-digit', month:'short', year:'numeric' });
  }

  if (!confirm(
    `Import this data?\n\nExported on: ${exportedOn}\nTransactions: ${txnCount}\nGoals: ${goalCount}\nCustom categories: ${catCount}\n\n⚠ This will MERGE with your existing data.\nDuplicate transactions will be skipped.`
  )) return;

  // ── Transactions: UUID-validate, whitelist every field, no spread ──
  const existingIds = new Set(STATE.transactions.map(t => t.id));
  const newTxns = (parsed.transactions || [])
    .filter(t => t.id && t.description && t.date && isUUID(t.id) && !existingIds.has(t.id))
    .map(t => ({
      id:          t.id,
      amount:      Math.abs(parseFloat(t.amount) || 0),
      isIncome:    t.isIncome === true,
      description: Security.sanitize(String(t.description || '').slice(0, 200)),
      category:    Security.sanitize(String(t.category    || 'Other').slice(0, 50)),
      type:        VALID_TYPES.has(t.type)          ? t.type        : 'Need',
      paymentMode: VALID_PAYMENTS.has(t.paymentMode) ? t.paymentMode : '',
      date:        safeDate(t.date),
      notes:       Security.sanitize(String(t.notes || '').slice(0, 300)),
      createdAt:   typeof t.createdAt === 'string'  ? t.createdAt   : new Date().toISOString()
    }))
    .filter(t => t.amount > 0);

  // ── Budgets: validate key and value, no blind spread ──
  if (parsed.budgets && typeof parsed.budgets === 'object' && !Array.isArray(parsed.budgets)) {
    Object.entries(parsed.budgets).forEach(([cat, val]) => {
      const safeCat = Security.sanitize(String(cat).slice(0, 50));
      const amount  = parseFloat(val);
      if (safeCat.length > 0 && !isNaN(amount) && amount > 0 && amount < 100000000 && !STATE.budgets[safeCat]) {
        STATE.budgets[safeCat] = amount;
      }
    });
  }

  // ── Goals: whitelist fields, no ...g spread ──
  const importedGoals = (Array.isArray(parsed.goals) ? parsed.goals : [])
    .slice(0, MAX_GOALS)
    .map(g => ({
      id:         isUUID(g.id) ? g.id : crypto.randomUUID(),
      name:       Security.sanitize(String(g.name    || 'Goal').slice(0, 100)),
    ...
    .filter(g => g.target > 0);
  STATE.goals = [...STATE.goals, ...importedGoals];

  // ── Categories: sanitize all fields, skip duplicates ──
  if (Array.isArray(parsed.categories)) {
    const existingNames = new Set(getUserCategories().map(c => c.name.toLowerCase()));
    const newCats = parsed.categories.slice(0, MAX_CATS)
      .filter(c => !c.builtIn && !existingNames.has((c.name || '').toLowerCase()))
      .map(c => ({
        name:    Security.sanitize(String(c.name || '').slice(0, 50)),
        icon:    Security.sanitizeIcon(c.icon || '🏷️'),
        color:   Security.validateHexColor(c.color) ? c.color : '#6366f1',
        builtIn: false
      }))
      .filter(c => c.name.length > 0);
    if (newCats.length > 0) STATE.settings.categories = [...getUserCategories(), ...newCats];
  }

  // ── Settings: strict type validation ──
  if (!STATE.settings.income) {
    const imp = parseInt(parsed.settings?.income);
    if (!isNaN(imp) && imp > 0 && imp < 100000000) STATE.settings.income = imp;
  }
  if (!STATE.settings.currency && VALID_CURRENCIES.has(parsed.settings?.currency))
  ...
  renderBudget();
  closeModal('profile-modal');
  showToast(`✅ Imported ${newTxns.length} transaction${newTxns.length !== 1 ? 's' : ''}${catCount > 0 ? `, ${catCount} categor${catCount !== 1 ? 'ies' : 'y'}` : ''}!`);
}
    ...
    <div class="modal-sheet">
      ...
      <div class="modal-body">
        <p class="auth-subtitle" style="margin-bottom:16px">
          Tap <strong>Use</strong> to fill your username into the login form.
        </p>
...
function confirmLogout() {
  if (confirm('Sign out of Expenseive?')) {
    ...
    // Clear ALL auth form fields — both login and register
    ['login-username', 'login-password',
     'reg-name', 'reg-username', 'reg-password', 'reg-confirm', 'reg-income'
    ].forEach(id => {
      const el = document.getElementById(id);
    ...
    // Always land on login form — never leave user on register form
    switchAuth('login');
    clearErrors();
    closeModal('profile-modal');
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Covers: modal close, txn card open, detail edit/delete, catmgr delete
  // Required because CSP blocks all inline onclick= handlers
  document.addEventListener('click', e => {
    // 1. Modal close (backdrops + ✕ buttons)
    const closeBtn = e.target.closest('[data-close]');
    if (closeBtn) { closeModal(closeBtn.dataset.close); return; }

    // 2. Transaction card — open detail
    const txnItem = e.target.closest('.txn-item[data-id]');
    if (txnItem) { openDetail(txnItem.dataset.id); return; }

    // 3. Edit / Delete inside detail modal
    const actionBtn = e.target.closest('[data-action]');
    if (actionBtn) {
      const id = actionBtn.dataset.txnId;
      if (actionBtn.dataset.action === 'edit')   { editTransaction(id); return; }
      if (actionBtn.dataset.action === 'delete') { deleteTransaction(id); return; }
    }

    // 4. Category manager delete
    const catDelBtn = e.target.closest('.catmgr-del-btn[data-cat-idx]');
    if (catDelBtn) { deleteCategory(parseInt(catDelBtn.dataset.catIdx, 10)); return; }
  });
