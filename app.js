...
function renderDashboard() {
  ...
  if (sortedCats.length === 0) {
    ...
    catList.innerHTML = sortedCats.map(([cat, amount]) => {
      ...
      const safeColor = Security.validateHexColor(getCategoryColor(cat)) ? getCategoryColor(cat) : '#6366f1';
...
function renderPaymentBreakdown(txns) {
  ...
  document.getElementById('payment-breakdown').innerHTML = sorted.length ? sorted.map(([mode, val]) => {
    const safeMode = Security.sanitizeLoaded(mode);           // VULN-04 fix
    const icon     = PAYMENT_ICONS[mode] || '';               // icon is a hardcoded emoji — safe
    return `
    <div class="payment-item">
      <span>${icon} ${safeMode}</span>
...
function renderBudget() {
  ...
  if (!Object.keys(STATE.budgets).length) {
    ...
    catBudgetsList.innerHTML = Object.entries(STATE.budgets).map(([cat, budget]) => {
      const safeCat    = Security.sanitizeLoaded(cat);                                    // VULN-05 fix
      const safeBudget = parseFloat(budget);                                              // VULN-05 fix — reject NaN/object
      if (isNaN(safeBudget) || safeBudget <= 0) return '';                               // skip corrupt entries
      const safeIcon   = Security.sanitizeIcon(getCategoryIcon(cat));
...
function exportData() {
  const data = {
    ...
    budgets:      STATE.budgets,
    goals:        STATE.goals,
    categories:   (STATE.settings.categories || []).filter(c => !c.builtIn), // custom only
...
async function handleImportFile(event) {
  ...
  try {
    ...
    showToast('⚠ Could not read file — must be valid JSON.'); return;
  }
  ...
  // ── Hard limits — prevent DoS via oversized files ──
  const MAX_TXNS  = 10000;
  const MAX_GOALS = 500;
  const MAX_CATS  = 100;
  if (parsed.transactions.length > MAX_TXNS) {
    showToast(`⚠ Import exceeds the ${MAX_TXNS} transaction limit.`); return;
  }

  // ── Allowlists for validated fields ──
  ...
  // VULN-07 fix — UUID validator, reject any non-UUID as an id
  const isUUID = v => typeof v === 'string' &&
    /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(v);

  // Safe date — must be YYYY-MM-DD
  const safeDate = v => /^\d{4}-\d{2}-\d{2}$/.test(v) ? v : todayStr();

  // Summary counts for confirmation dialog
  ...
  // exportedAt — parse through Date, never trust raw string (VULN-06 adjacent)
  ...
  const newTxns = (parsed.transactions || [])
    ...
    .map(t => ({
      ...
      type:        VALID_TYPES.has(t.type)     ? t.type        : 'Need',              // allowlist
      paymentMode: VALID_PAYMENTS.has(t.paymentMode) ? t.paymentMode : '',            // allowlist
  ...
  // ── VULN-02 fix: validate each budget key AND value — no blind spread ──
  if (parsed.budgets && typeof parsed.budgets === 'object' && !Array.isArray(parsed.budgets)) {
    Object.entries(parsed.budgets).forEach(([cat, val]) => {
      ...
      const amount  = parseFloat(val);                                                  // rejects NaN, Infinity, objects
      if (safeCat.length > 0 && !isNaN(amount) && amount > 0 && amount < 100000000) {
        if (!STATE.budgets[safeCat]) STATE.budgets[safeCat] = amount;                  // existing take priority
      }
    });
  }

  // ── VULN-01 fix: whitelist goal fields — NO ...g spread ──
  ...
  // ── Categories — sanitize every field ──
  if (Array.isArray(parsed.categories)) {
    ...
    if (newCats.length > 0)
      STATE.settings.categories = [...getUserCategories(), ...newCats];
  }

  // ── VULN-06 fix: strictly validate settings types ──
  ...
  closeModal('profile-modal');
  showToast(
    `✅ Imported ${newTxns.length} transaction${newTxns.length !== 1 ? 's' : ''}` +
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Handles: modal close, txn-item open, detail edit/delete, catmgr delete
  // Replaces ALL inline onclick handlers — required by CSP script-src 'self'
  document.addEventListener('click', e => {
    // 1. Modal close (backdrops + ✕ buttons with data-close)
    const closeBtn = e.target.closest('[data-close]');
    if (closeBtn) { closeModal(closeBtn.dataset.close); return; }

    // 2. Transaction card — open detail modal
    ...
    // 4. Category manager delete button
